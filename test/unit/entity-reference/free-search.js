...
describe("BridgeDb.EntityReference.freeSearch", function() {
  ...
  //*
  it("should free search for entity references (English/single instance)", function(done) {
    var testCoordinator = this;
    var test = this.test;
    test.expectedPath = __dirname + "/hits-for-agt-mouse.jsonld";
    var expected = JSON.parse(
      fs.readFileSync(test.expectedPath, { encoding: "utf8" })
    );
    ...
    bridgeDb1.entityReference
      ...
      .map(function(actualXrefs) {
        return JSON.parse(
          JSON.stringify(actualXrefs).replace(
            new RegExp(bridgeDb1.config.baseIri, "g"),
            standardBridgeDbApiUrlStub
          )
        ).sort(sortById);
