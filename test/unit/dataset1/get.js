...
describe('BridgeDb.Dataset._getAll', function() {
  var allPassed = true;
  var that = this;
  var update;
  var lkgDataPath;
  ...
  it('should get metadata by provider @id', function(done) {
    ...
    bridgeDbInstance.dataset._getAll({
    ...
    .let(function(source) {
      if (update) {
        return source.let(RxFs.createWriteObservable(lkgDataPath));
      } else {
        return source;
      }
    })
