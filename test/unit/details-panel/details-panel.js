...
describe("DetailsPanel", function() {
  ...
  //*
  it("should get xrefs by bridgeDbXrefsIri, formatted for display", function(done) {
    var lkgDataPath = __dirname + "/ncbigene-1234-xrefs.jsonld";
    ...
    var xrefStream = bridgeDbInstance.xref
      .get({
        bridgeDbXrefsIri:
          "http://webservice.bridgedb.org/Homo sapiens" + "/xrefs/L/1234"
      })
      .collect()
      .map(function(currentXrefs) {
        return JSON.stringify(currentXrefs).replace(
          new RegExp(bridgeDbInstance.config.baseIri, "g"),
          standardBridgeDbApiUrlStub
        );
      })
      .pipe(
        highland.pipeline(function(s) {
          if (update) {
            s.fork()
