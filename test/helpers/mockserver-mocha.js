...
var getFreePort = Rx.Observable.bindNodeCallback(freeport);
  ...
  global.before(function(next) {
    ...
    if (typeof process.env.MOCKSERVER_PORT !== "undefined") {
      ...
      portSource = Rx.Observable.fromEvent(isBusyServer, "error")
        ...
        .timeout(
          ...
          400,
          Rx.Observable.return(process.env.MOCKSERVER_PORT).do(function() {
            isBusyServer.close();
          })
        );
    } else {
    ...
    portSource
      ...
      .subscribe(
        function(port) {
          server = http
            .createServer(mockserver(__dirname + "/input-data/"))
            .listen(port);
          setTimeout(function() {
            next(null);
          }, 1500);
        ...
        function(err) {
          err.message =
            (err.message || "") + ' in "before" for mockserverMocha';
          next(err);
        }
      );
  });
