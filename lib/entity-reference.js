...
// TODO Change this to use hyperquest instead
...
var EntityReference = function(instance) {
   ...
   * @property {Array<String>} xrefs List of IRIs (URLs) for getting Xrefs,
  ...
  */
   ...
   * keeping any existing xrefs elements.
   *
   * @private
   *
   * @param {EntityReference} entityReference
   ...
   * @param {Dataset} entityReference.isDataItemIn
   * @param {Array<String>} entityReference.isDataItemIn.alternatePrefix The first element
   *                    must be the BridgeDb systemCode
  ...
  var _addBridgeDbXrefsIri = function(entityReference) {
    ...
    // entityReference.xrefs in all cases, including the case
    // where entityReference.xrefs was just defined as an
    // empty array above.
   ...
   * with the exception of not dereferencing any xrefs IRIs, but this enrichment can be controlled
   ...
   *        b. { bridgeDbXrefsIri: BridgeDb xrefs IRI }
   *        c. { xrefs: [
   *               BridgeDb xrefs IRI
   *               ...
   *             ]
    ...
    return inputStream
      ...
      .consume(function(err, entityReference, push, next) {
        if (err) {
          ...
          console.log('entityReference233');
          console.log(entityReference);
          var bridgeDbInputOriginalContext = entityReference['@context'];
          console.log('bridgeDbInputOriginalContext233');
          console.log(bridgeDbInputOriginalContext);
          jsonldRx.replaceContext(entityReference, internalContext)
            .subscribe(function(entityReference) {
              ...
              console.log('entityReference234');
              console.log(entityReference);
      ...
      .consume(function(err, entityReference, push, next) {
        if (err) {
          ...
          console.log('bridgeDbInputOriginalContext323');
          console.log(bridgeDbInputOriginalContext);
          console.log('entityReference323');
          console.log(entityReference);
          // TODO can we get rid of this checking to ensure removal of empty type values?
          var entityReferenceType = entityReference['@type'];
          if (entityReferenceType) {
