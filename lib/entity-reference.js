...
//  };
   ...
   *
   * @param {(Object|String|Observable)} input Entity reference(s). Each one must have an identifier
  ...
  function enrich(input, options) {
    ...
    } else if (input.hasOwnProperty('concatMap')) {
      // TODO get a better test for whether the current input is an Observable
      inputSource = input;
    }
    options = options || {};
    ...
    .doOnError(function(err) {
      ...
      err.message += ' in BridgeDb.EntityReference.enrich';
      throw err;
    });
  }
  ...
  function _enrichFromDataset(entityReference) {
    ...
    .doOnError(function(err) {
      err.message = err.message || '';
      err.message += ' in BridgeDb.EntityReference._enrichFromDataset';
      throw err;
    });
  }
  ...
  function exists(systemCode, identifier, organism) {
    ...
    .flatMap(function(organismName) {
      ...
      return Rx.Observable.fromUnpausableStream(
      ...
      .map(function(buf) {
        ...
        if (str === 'true') {
          ...
          throw new Error('Unrecognized response: "' + buf.toString() + '"');
        }
      });
    })
    .doOnError(function(err) {
      err.message = err.message || '';
      err.message += ' in BridgeDb.EntityReference.exists';
      throw err;
    });
  }
  ...
  function freeSearch(args) {
    ...
    .doOnError(function(err) {
      err.message = err.message || '';
      err.message += ' in BridgeDb.EntityReference.freeSearch';
      throw err;
    })
    .map(function(array) {
      var result = {
  ...
  function map(args) {
    ...
    .doOnError(function(err) {
      err.message = err.message || '';
      err.message += ' in BridgeDb.EntityReference.map';
      throw err;
    });
  }
  ...
  function normalize(entityReference) {
    ...
    if (!!organism) {
      ...
      .doOnError(function(err) {
        err.message = err.message || '';
        err.message += ' in BridgeDb.EntityReference.normalize';
        throw err;
      });
    } else {
      return Rx.Observable.return(entityReference)
