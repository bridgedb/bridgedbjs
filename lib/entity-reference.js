...
var EntityReference = function(instance) {
   ...
   * @property {Array<String>} xref IRI (URL) for getting Xrefs, such as from the BridgeDb webservices.
   * @property {String} identifier See {@link http://www.biopax.org/release/biopax-level3.owl#id|biopax:id} Example: "1234".
  ...
  */
   ...
   * @return {EntityReference} entityReference {@link EntityReference} with BridgeDb IRI (URL) added.
   */
  var _addBridgeDbXrefsIri = function(entityReference) {
    if (!entityReference ||
        !entityReference.organism ||
        !entityReference.isDataItemIn ||
        !entityReference.isDataItemIn.alternatePrefix ||
        !entityReference.identifier) {
      if (instance.debug) {
   ...
   * @param {Boolean} [options.bridgeDbXrefsIri=true] Enrich with IRI (URL) for BridgeDb webservices
   ...
   *      1. BridgeDb xrefs IRI (URL) or identifiers.org IRI as string
   *      2. Object with at least one of these properties:
   *        a. { '@id': identifiers.org IRI }
   *        b. { bridgeDbXrefsIri: BridgeDb xrefs IRI (URL) }
   *        c. { xref: [
   *            BridgeDb xrefs IRI (URL)
   *            ...
   *          ]
   *        }
   ...
   *        }
   *
   *    Example of BridgeDb xrefs IRI (URL): http://webservice.bridgedb.org/Human/xrefs/L/1234
   ...
   *                         (metadata about biological datasets).
   * @param {Boolean} [options.bridgeDbXrefsIri=true] Enrich with IRI (URL) for BridgeDb webservices
   *                                        to enable getting xrefs for this entity reference. Note
   *                                        that if you set this to true, you will get the organism
   *                                        regardless of what you set for options.organism, because
   *                                        we need to get the organism in order to get this.
  ...
  var enrich = function(input, options) {
    ...
    .map(function(entityReference) {
      if (options.bridgeDbXrefsIri) {
        return _addBridgeDbXrefsIri(entityReference);
      } else {
        return entityReference;
      }
      //*/
    ...
    return datasetsStream.map(function(dataset) {
      ...
      if (!_.isEmpty(typeFromDataset)) {
        ...
        entityReference['@type'] = _.union(
          Utils._arrayify(entityReference['@type']), typeFromDataset);
      }
   ...
   * @param {String|Organism} organism {@link Organism} or name in English or Latin or taxonomy IRI like
   *                          {@link http://identifiers.org/taxonomy/9606|http://identifiers.org/taxonomy/9606}.
   ...
   * @param {String|Organism} organism {@link Organism} or name in English or Latin or taxonomy IRI like
   *                          {@link http://identifiers.org/taxonomy/9606|http://identifiers.org/taxonomy/9606}.
   * @param {JsonldType} [args['@type']] - Entity reference type, such as Protein, Dna, SmallMolecule, etc.
