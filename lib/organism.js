...
//  };
  ...
  function _getInstanceOrganism(searchCriteria) {
    ...
    instance.organismNormalizedSource = organismNormalizedSource.replay();
   ...
   * @param {Boolean} [normalize=true] Normalize the provided organism to ensure it matches what
   *                                 the BridgeDb API expects.
   * @return {Observable<Organism>} Normalized organism name. Note that if args.normalize is
  ...
  function _setInstanceOrganism(organism, normalize) {
    ...
    if (normalize) {
      ...
      .doOnError(function(err) {
        err.message = err.message || '';
        err.message += ' in BridgeDb.Organism._setInstanceOrganism';
        throw err;
      });
    } else {
      return Rx.Observable.return(organism)
      .doOnError(function(err) {
        err.message = (err.message || '') + 'in BridgeDb.Organism._setInstanceOrganism';
        throw err;
      });
    }
  }
