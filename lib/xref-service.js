  ...
  var createStream = function() {
    return highland.pipeline(function(sourceStream) {
      ...
      .flatMap(function(normalizedEntityReference) {
        var source = getBridgeDbUrlByEntityReference(specifiedEntityReference);
  ...
  function formatAllForDisplay(args) {
    ...
    .map(function(entityReference) {
      ...
      if (entityReference.hasOwnProperty('linkoutPattern')) {
        listItem.uri =
          entityReference.linkoutPattern.replace('$id', identifier);
      }
      return listItem;
    })
    .collect()
    ...
    .map(function(listItems) {
      ...
      if (listItems.length < 2) {
        ...
        if (specifiedEntityReference.hasOwnProperty('linkoutPattern')) {
          uri = specifiedEntityReference.linkoutPattern.replace(
            '$id',
