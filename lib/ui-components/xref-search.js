...
function XrefSearch(args) {
  ...
  props.query = (props.query || Rx.Observable.return(xrefsPlaceholder));
  var xrefList = Rx.Observable.merge(
      props.xrefs,
      handleChange
...
//        },
      ...
      xrefList.map(function(xrefs) {
        console.log('xrefs236');
        console.log(xrefs);
        var content;
        // TODO:
        // 1) don't display the modal immediately. wait for the query and then display.
        // 2) get the onclick stuff working.
        //showEntityReferences.onNext(false);
//        if (xrefs && showEntityReferences) {
//        }
        if (xrefs) {
          if (xrefs.length > 0) {
