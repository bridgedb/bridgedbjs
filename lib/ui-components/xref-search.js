...
function XrefSearch(args) {
  ...
  .concatMap(function(selectedEntityReferenceId) {
    return xrefsSubject$.map(function(xrefs) {
      return _.find(xrefs, function(xref) {
    ...
    h('div.input-group.input-group-sm.form-control', null,
      ...
      .map(function(result) {
        ...
        var content = bridgeDbInstance.entityReference.freeSearch(
            {attribute: query})
        .timeout(45 * 1000) // 45s seems really long, but sometimes the BridgeDb webservice is slow
