...
function XrefSearch(args) {
  ...
  var handleDummyInput = createEventHandler();
  var queryValueFromDummy$ = handleDummyInput
  .map(function(x) {
    return x;
  })
  //.debounce(300)
  ...
  var queryValueFromField$ = handleQueryFieldChange
  .map(function(x) {
    return x;
  })
  //.debounce(300)
  ...
  var query$ = Rx.Observable.merge(
      querySubject$,
      props.query || Rx.Observable.return(''),
      queryValueFromField$
  )
  ...
  var xrefsList$ = Rx.Observable.merge(
      xrefsSubject$,
      query$
      .withLatestFrom(organism$)
      .flatMap(function(result) {
        var query = result[0];
        var organism = result[1];
...
//  //.startWith({})
  ...
  return h('div.form-search.form-group', null,
    h('span', null, 'query: '),
    h('span', null, queryValueFromDummy$),
//    h('span', null, disabled$
//      .withLatestFrom(query$)
//      .map(function(result) {
//        var disabled = result[0];
//        var query = result[1];
//        return [
//          'disabled? ' + disabled,
...
//    ),
    h('input', {
      onChange: handleDummyInput,
      'placeholder': 'dummy',
    }),
    h('span', null, queryValueFromField$),
