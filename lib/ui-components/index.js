...
function BridgeDbUIElement(args) {
  ...
  //return h('p', null, 'hmmm');
  return h('p', null, organismIn$);
  ...
  .subscribe(function(bridgeDbConfig) {
    console.log('bridgeDbConfig');
    console.log(bridgeDbConfig);
    bridgeDbInstance$.onNext(new BridgeDb(bridgeDbConfig));
  }, function(err) {
    console.error('error getting bridgeDbConfig');
    throw err;
  });
  ...
  entityIn$
  .doOnNext(function(entity) {
    console.log('entity211');
    console.log(entity);
  })
