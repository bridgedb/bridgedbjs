 ...
 *   datasetsMetadataIri:
 *    'https://raw.githubusercontent.com/bridgedb/BridgeDb/master/org.bridgedb.bio/
 *        resources/org/bridgedb/bio/datasources.txt'  // Optional
 * });
 *
 ...
 *    a proxy for now for CORS so that browsers can access the data.
 * @param {string} [options.datasetsMetadataIri=
 *    'http://pointer.ucsf.edu/d3/r/data-sources/bridgedb-datasources.php'] Location
 ...
 *    It should be either proxied for bridgedb.org or else directly the following source:
 *    'https://raw.githubusercontent.com/bridgedb/BridgeDb/master/org.bridgedb.bio/
 *        resources/org/bridgedb/bio/datasources.txt'
 *    but we are using a proxy for now for CORS.
...
var BridgeDb = function(options) {
  ...
  instance.addContext = function(inputDoc) {
    ...
    // The section below was attempting to merge multiple contexts, in
    // the case that the input document already had a context.
    // In that case, we would want to prioritize the input document's
    // terms over the bridgedbjs terms.
    // TODO finish this
    /*
    var externalContextAfterElementsStringified =
      externalContext.map(JSON.stringify);

    var unionContext = internalContext.filter(function(internalContextElement) {
      return externalContextAfterElementsStringified.indexOf(
        JSON.stringify(internalContextElement)) === -1;
    })
