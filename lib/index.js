...
var BridgeDb = (function(){
  ...
  if (!metadata) {
    MetadataService.get(function(err, returnedMetadata) {
      ...
      MetadataService.metadata = returnedMetadata;
      metadataLoaded.emit('load', returnedMetadata);
    });
  }
  ...
  function getBridgedbApiHostAndPathByEntityReference(args, callback) {
    ...
    if (!!args.bridgedbUri) {
      ...
      getMetadataForSystemCode(systemCode, function(err, metadataRow) {
      ...
      if (!!identifiersIdResults && identifiersIdResults.length > 0) {
        ...
        getMetadataForIdentifiersNamespace(identifiersNamespace, function(err, metadataForIdentifiersNamespace) {
  ...
  function getBridgedbSystemCode(dbName, callback) {
    getMetadataForDbName(dbName, function(err, metadataForDbName) {
  ...
  function getXrefsNestedForDisplay(args, callback) {
    ...
    getXrefs(args, function(err, entityReferenceXrefs) {
      ...
      var createListItem = function(entityReferenceXref, mapCallback){ 
        ...
        getMetadataForDbName(entityReferenceXref.dbName, function(err, metadataForDbName) {
      ...
      async.map(entityReferenceXrefs, createListItem, function(err, listItems){
        ...
        if (nestedListItems.length < 2) {
          var uri = '';
          getMetadataForDbName(providedDbName, function(err, metadata) {
  ...
  function getMetadataForIdentifiersNamespace(identifiersNamespace, callback) {
    async.waterfall([
      function(waterfallCallback) {
        getMetadata(function(err, metadata) {
          console.log('getMetadataForIdentifiersNamespace');
          waterfallCallback(null, metadata);
        });
      },
  ...
  function getMetadataForDbName(dbName, callback) {
  ...
  function getMetadataForSystemCode(systemCode, callback) {
    async.waterfall([
      function(waterfallCallback) {
        getMetadata(function(err, metadata) {
          waterfallCallback(null, metadata);
      ...
      function(metadata, waterfallCallback) {
        var metadataForSystemCodeResults = metadata.filter(function(row) { return row.systemCode === systemCode; });
      ...
      var callback = function(response) {
        ...
        response.on('end', function () {
          var enrichXref = function(xref, enrichXrefCallback) {
            getMetadataForDbName(xref.dbName, function(err, metadataRow) {
              if (!!metadataRow.type) {
