...
var Metadata = function(instance) {
  ...
  var getByDbWithAllArguments = function(db, metadata) {
    ...
    /*
    var metadataStream = highland(metadata);
    var metadataStrictMatch = metadataStream.fork().filter(function(row) {
      return row.db === db;
    });

    // second attempt. if first attempt failed, we get a little looser about the match here on the second attempt.
    var metadataLooseMatch = metadataStream.fork().filter(function(row) {
      return Utilities.normalizeText(row.db) === Utilities.normalizeText(db);
    });

    // no third attempt. if second attempt failed, we give up.
    var metadataNoMatch = metadataStream.fork().filter(function(row) {
      return new Error('No BridgeDB database metadata returned for db "' + db + '"');
    });

    return metadataStrictMatch.concat(metadataLooseMatch)
    .concat(metadataNoMatch)
    .head();
    //*/
  };
  ...
  var getByEntityReference = function(entityReference) {
    ...
    if (!bridgedbSystemCode) {
      entityReferenceStream = highland([instance.entityReferenceService.expand(entityReference)])
      .flatMap(function(entityReference) {
        db = entityReference.db;
        if (!db) {
          entityReferenceStream.destroy();
          throw new Error('No db and no bridgedbSystemCode provided. Cannot return metadata.');
        }
        return getBridgedbSystemCodeByDb(db);
      })
