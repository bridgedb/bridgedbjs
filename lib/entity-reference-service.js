...
var EntityReference = function(instance) {
  ...
  var enrich = function(entityReference, metadata) {
    ...
    .flatMap(function(entityReference) {
      console.log('entityReference427');
      console.log(entityReference);
      ...
      .map(function(entityReference) {
        console.log('entityReference432');
        console.log(entityReference);
        return entityReference.id;
      })
      .flatMap(instance.organismService.getByIri)
      .map(function(organism) {
        console.log('organism428');
        console.log(organism);
        entityReference.organism = organism.latin || organism.english;
        return entityReference;
      });
    });

    /*
    return enrich(entityReference).flatMap(function(entityReference) {
      // TODO add method to set organism, if it's specified, so we don't need to run this method below
      // same for getByIdentifiersDotOrgIri
      return highland([entityReference]).flatMap(instance.organismService.getByIri)
      .map(function(organism) {
    ...
    /*
    return instance.metadataService.getByEntityReference(entityReference).map(function(metadataRow) {
      console.log('metadataRow404');
      console.log(metadataRow);
      delete metadataRow.exampleIdentifier;
      delete metadataRow.website;
      var entityReferenceWithDefaults = _.defaults(entityReference, metadataRow);
      return addIdentifiersDotOrgIri(entityReferenceWithDefaults);
    });
    //*/
  };
  ...
  function getByIri(args) {
    ...
    return instance.metadataService.getByIdentifiersDotOrgNamespace(entityReference.namespace).flatMap(function(metadataRow) {
      return enrich(entityReference, metadataRow);
    });
  }
      ...
      .consume(function(err, searchResult, push, next) {
        if (err) {
          ...
          instance.metadataService.getByDb(searchResult.db).each(function(metadataRow) {
            ...
            console.log('enrichedSearchResult514');
            console.log(enrichedSearchResult);
            push(null, enrichedSearchResult);
          });
          //*/
