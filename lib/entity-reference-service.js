...
var EntityReferenceService = function(instance) {
  ...
  /**
   * Enrich entity reference as much as possible from BridgeDB, with the exception of not getting xrefs.
   *
   * @param {object} entityReference
   * @param {object} [databases]
   * @return {Stream<object>} entityReference Entity reference with as many as possible of the properties listed below.
   * @return {string} entityReference['@context'] JSON-LD context. You can ignore this if you don't care about semantics.
   ...
   */
  var enrich = function(entityReference, databases) {
    return enrichWithDatabaseMetadata(entityReference, databases)
   ...
   * @private
   *
   * Enrich provided entity reference with all data possible using the databases metadata
   * from datasources.txt.
   *
   * @param {object} entityReference
   * @param {object} [databases]
   * @return {Stream<object>} entityReference Entity reference enriched with database metadata
   */
  function enrichWithDatabaseMetadata(entityReference, databases) {
    var databasesStream;
    if (!!databases) {
      databasesStream = highland([ databases ]);
    } else {
      databasesStream = instance.databaseService.getByEntityReference(entityReference);
    }
   ...
   *
   * @param {object|string} entityReference @see bridgedb.entityReferenceService.get for details on what constitutes a usable entityReference
   ...
   *
   * @return {Stream<object>} entityReference Entity reference, enriched with
   *                          databases metadata and organism
   * @return {string} entityReference.identifier
   * @return {string} entityReference.PLUS_ALL_OTHER_PROPERTIES_AVAILABLE
   */
  function get(args) {
   ...
   * @param {string|object} args.sourceEntityReference @see bridgedbjs.entityReferenceService.get
   ...
   * @return {Stream<object>} entityReferenceStream If there is one or more matching entity references,
   *                           they will be returned as objects, as described below.
   * @return {object} entityReferenceStream.entityReference Entity reference, enriched with databases metadata and organism
   */
