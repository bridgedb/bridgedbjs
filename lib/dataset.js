...
//require('rx-extra/lib/stream-through.js')(Rx);
...
var Dataset = function(instance) {
  ...
  function _getAll() {
    ...
    .map(function(array) {
      ...
      result[BDB + 'hasDataSource'] = array[0];
      result[BDB + 'systemCode'] = array[1];
      result.uri = array[8];
    ...
    .doOnError(function(err) {
      ...
      err.message += ' in BridgeDb.Dataset._getAll';
      throw err;
    })
    .shareReplay();
  }
  ...
  function _getAllProcessedForMatcher() {
    ...
    if (datasetsProcessedForMatcher) {
      ...
      .doOnError(function(err) {
        err.message = (err.message || '') +
            ' in BridgeDb.Dataset._getAllProcessedForMatcher (cached)';
        throw err;
      });
      ...
      .doOnError(function(err) {
        err.message = (err.message || '') +
            ' in BridgeDb.Dataset._getAllProcessedForMatcher (getting)';
        throw err;
      });
    }
    ...
    .doOnError(function(err) {
      err.message = (err.message || '') + 'in BridgeDb.Dataset._getAllProcessedForMatcher';
      throw err;
    });
  }
  ...
  function get(args) {
    ...
    .doOnError(function(err) {
      err.message = err.message || '';
      err.message += ' in BridgeDb.Dataset.get';
      throw err;
    });
  }
  ...
  function query(args) {
    if (_.isEmpty(args)) {
      ...
      .doOnError(function(err) {
        err.message = err.message || '';
        err.message += ' in BridgeDb.Dataset.query';
        throw err;
      });
