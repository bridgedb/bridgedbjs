  ...
  function getBridgedbApiPathByEntityReference(args, callback) {
    ...
    if (!!args.organism && (!!args.bridgedbSystemCode || !!args.db) && typeof args.identifier !== 'undefined') {
      ...
      if (!!identifiersIdResults && identifiersIdResults.length > 0) {
        ...
        instance.metadataService.getByIdentifiersDotOrgNamespace(namespace).each(function(metadataRow) {
          ...
          instance.metadataService.getBridgedbSystemCodeByDb(db).each(function(bridgedbSystemCode) {
            instance.organismService.getNameByIri(identifiersId).each(function(organismName) {
              console.log('200organismName');
              ...
              path = '/' + encodeURIComponent(organismName) + '/xrefs/' + encodeURIComponent(bridgedbSystemCode) + '/' + encodeURIComponent(identifier);
              return callback(null, path);
            });
          });
        });
