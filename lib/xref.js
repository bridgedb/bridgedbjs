...
var Xref = function(instance) {
   ...
   * @private
   *
   * Get Xrefs in tab-separated-value format.
   * This is only split out as its own function so that it can be
   * mocked in testing.
   *
   * @return {Stream<string>} data sources. Stream is chunked.
   */
  function getAsTsv(entityReference) {
    var source = getBridgeDbUrlByEntityReference(entityReference);
    return request({
      url: source,
  ...
  function createStream() {
    return highland.pipeline(function(sourceStream) {
      ...
      .flatMap(function(normalizedEntityReference) {
        return highland(
          getAsTsv(specifiedEntityReference).pipe(csv(csvOptions))
        );
      })
      .map(function(array) {
        return array;
