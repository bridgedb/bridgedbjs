...
var Xref = function(instance) {
  ...
  function getBridgedbApiPathByEntityReference(args, callback) {
    ...
    //*/
    if (!!args.organism && (!!args.systemCode || !!args.db) && typeof args.identifier !== 'undefined') {
      providedDb = args.db;
      providedIdentifier = args.identifier;
      if (!!args.systemCode) {
        path = '/' + encodeURIComponent(args.organism) + '/xrefs/' + encodeURIComponent(args.systemCode) + '/' + encodeURIComponent(args.identifier);
        return callback(null, path);
      } else {
        instance.metadataService.getSystemCodeByDb(args.db, function(err, systemCode) {
          path = '/' + encodeURIComponent(args.organism) + '/xrefs/' + encodeURIComponent(systemCode) + '/' + encodeURIComponent(args.identifier);
          return callback(null, path);
        });
      }
    } else {
      ...
      if (!!identifiersIdResults && identifiersIdResults.length > 0) {
        ...
        instance.metadataService.getByIdentifiersNamespace(identifiersNamespace).each(function(metadataForIdentifiersNamespace) {
          ...
          instance.metadataService.getSystemCodeByDb(db).each(function(systemCode) {
            instance.organismService.getByIdentifier(identifiersId).each(function(organism) {
              ...
              path = '/' + encodeURIComponent(organism.latin) + '/xrefs/' + encodeURIComponent(systemCode) + '/' + encodeURIComponent(identifier);
              return callback(null, path);
            });
          });
        });
  ...
  function exists(systemCode, identifier, organism, callbackOutside) {
    var path = '/' + encodeURIComponent(organism) + '/xrefExists/' + systemCode + '/' + identifier;
    var source = instance.config.apiUrlStub + path;
